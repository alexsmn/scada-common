...
inline bool BasicTimedDataBuffer<T>::InsertOrUpdate(const T& value) {
  ...
  if (scada::IsNull(timestamp(value)))
    return false;
...
inline void BasicTimedDataBuffer<T>::ClearRange(
  ...
  auto j = scada::IsNull(range.second) ? values_.size()
