...
class TestAddressSpace : public scada::ViewService {
 public:
  ...
  bool IsSubtypeOf(const scada::NodeId& type_id,
  ...
  const scada::NodeId kHasTypeDefinitionRefTypeId{"kHasTypeDefinitionRefTypeId",
                                                  0};
  ...
  const std::vector<scada::ReferenceState> kReferences{
      {kTestRefTypeId, kTestNode1.node_id, kTestNode2.node_id},
  };

 private:
  bool Matches(const scada::ReferenceDescription& reference,
               const scada::BrowseDescription& description) const;
  void ReturnIfMatches(std::vector<scada::ReferenceDescription>& references,
                       const scada::BrowseDescription& description,
                       scada::ReferenceDescription reference) const;
};

inline const scada::BrowseNode* TestAddressSpace::GetNode(
...
inline scada::BrowseResult TestAddressSpace::Browse(
  ...
  if (!node->type_id.is_null())
    ReturnIfMatches(references, description,
                    {scada::id::HasTypeDefinition, false, node->type_id});
  if (!node->reference_type_id.is_null())
    ReturnIfMatches(references, description,
                    {node->reference_type_id, false, node->parent_id});

  for (auto& child_node : kNodes) {
    if (child_node.parent_id == node->node_id)
      ReturnIfMatches(references, description,
                      {child_node.reference_type_id, true, child_node.node_id});
  }
  ...
  for (auto& ref : kReferences) {
    ...
    ReturnIfMatches(references, description,
                    {ref.reference_type_id, forward, node_id});
  }
...
inline bool TestAddressSpace::IsSubtypeOf(
    const scada::NodeId& type_id,
    const scada::NodeId& supertype_id) const {
  for (auto node_id = type_id; !node_id.is_null();
       node_id = GetSupertypeId(node_id)) {
...
inline bool TestAddressSpace::Matches(
  ...
  bool reference_type_matches =
      description.include_subtypes
          ? IsSubtypeOf(reference.reference_type_id,
...
inline void TestAddressSpace::ReturnIfMatches(
    std::vector<scada::ReferenceDescription>& references,
    const scada::BrowseDescription& description,
    scada::ReferenceDescription reference) const {
...
void TestAddressSpace::TranslateBrowsePath(
    const scada::NodeId& starting_node_id,
    const scada::RelativePath& relative_path,
    const scada::TranslateBrowsePathCallback& callback) {
