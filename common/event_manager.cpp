...
EventManager::EventManager(EventManagerContext&& context)
  ...
  monitored_item_ = monitored_item_service_.CreateMonitoredItem(OpcUaId_RootFolder, OpcUa_Attributes_EventNotifier);
  assert(monitored_item_);
  monitored_item_->set_event_handler([this](const scada::Event& event) {
      OnEvent(event);
  });
...
void EventManager::SetSeverityMin(unsigned severity) {
  ...
  for (ObserverSet::const_iterator i = observers_.begin(); i != observers_.end(); )
...
void EventManager::AddUnackedEvent(const scada::Event& event) {
  ...
  std::pair<EventContainer::iterator, bool> p =
      unacked_events_.insert(EventContainer::value_type(event.acknowledge_id, event));
...
void EventManager::RemoveUnackedEvent(const scada::Event& event) {
  ...
  // Acknowledge confirmation.
  {
    ...
    if (i != running_ack_event_ids_.end()) {
      ...
      logger().WriteF(LogSeverity::Normal, "Event %d acknowledged", event.acknowledge_id);
  ...
  if (!contained_event.node_id.is_null()) {
    ItemEventMap::iterator i = item_unacked_events_.find(contained_event.node_id);
    if (i != item_unacked_events_.end()) {
      ...
      ItemEventsChanged(observers_, contained_event.node_id, item_event_data.events);
...
void EventManager::AcknowledgeEvent(unsigned ack_id) {
  ...
  if (std::find(pending_ack_event_ids_.begin(),
                pending_ack_event_ids_.end(), ack_id) != pending_ack_event_ids_.end())
    return;
...
void EventManager::Update() {
  ...
  history_service_.HistoryRead({OpcUaId_RootFolder, OpcUa_Attributes_EventNotifier}, {}, {}, {{scada::Event::UNACKED}},
...
void EventManager::OnQueryEventsResult(scada::Status status, scada::QueryEventsResults results) {
  if (!results)
    return;
