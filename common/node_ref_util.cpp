...
void BrowseNodesRecursive(scada::ViewService& view_service,
  ...
  struct Browser : public std::enable_shared_from_this<Browser> {
    ...
    void Browse(const scada::NodeId& parent_id) {
      ...
      BrowseNodes(view_service, node_service,
                  {parent_id, scada::BrowseDirection::Forward,
                   scada::id::HierarchicalReferences, true},
                  [self, this](const scada::Status& status,
                               const std::vector<NodeRef>& nodes) {
                    for (auto& node : nodes) {
                      if (type_definition_id_.is_null() ||
                          IsInstanceOf(node, type_definition_id_))
  ...
  std::make_shared<Browser>(view_service, node_service, type_definition_id,
                            callback)
      ->Browse(parent_id);
}
...
void BrowseInstanceDeclarations(scada::ViewService& view_service,
                                NodeService& node_service,
                                const scada::NodeId& type_definition_id,
  ...
  BrowseNodes(
      ...
      {type_definition_id, scada::BrowseDirection::Forward, reference_type_id,
       true},
      [&view_service, &node_service, type_definition_id, callback,
       reference_type_id](const scada::Status& status,
                          std::vector<NodeRef> declarations) {
        auto declarations_ptr =
            std::make_shared<std::vector<NodeRef>>(std::move(declarations));
        // Request supertype.
        BrowseReference(
            view_service,
                  ...
                  [declarations_ptr,
                   callback](std::vector<NodeRef> super_declarations) {
                    declarations_ptr->insert(declarations_ptr->begin(),
                                             super_declarations.begin(),
                                             super_declarations.end());
                    callback(std::move(*declarations_ptr));
                  });
            });
      });
