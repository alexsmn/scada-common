...
// the namespace short names). They must therefore never change: a `symbolicName`
// in a nodeset may rename the C++ handle, but the underlying id is frozen. This
// test pins those values so an accidental renumbering (e.g. editing a nodeset id
// or reordering the namespace manifest) fails the build instead of silently
// breaking every existing database.
//
// When you intentionally ADD a node, add its constant here too; you should never
// need to CHANGE an existing expectation.
...
// The namespace short names double as configuration-DB table names and must stay
// byte-for-byte identical in this exact index order.
TEST(ModelFrozenIds, NamespaceShortNames) {
  static constexpr std::string_view kExpected[] = {
      ...
      "HISTORY",   "SIM_ITEM",     "IEC_LINK",      "IEC_DEV",
      "MODBUS_PORTS", "DEVICES",   "TS_PARAMS",     "SERVER_PARAMS",
      "IEC_TRANSMIT", "IEC61850_DEV", "IEC61850_RCB", "FILESYSTEM",
      "DATA_ITEMS", "SECURITY",    "IEC61850_SERVER", "ALIAS",
      "FILESYSTEM_FILE", "OPC",    "MODBUS_TRANSMIT", "IEC61850_TRANSMIT",
      "VIDICON",   "VIDICON_FILE", "ROLE",          "ROLE_IDENTITY",
      "CONFIGURATION"};
...
// Namespace indices are also persisted (they are the numeric namespace of stored
// NodeIds). Pin the ones exposed as C++ constants.
TEST(ModelFrozenIds, NamespaceIndices) {
  ...
  EXPECT_EQ(NamespaceIndexes::END, 33);
}
...
TEST(ModelFrozenIds, NodeIdValues) {
  ...
  EXPECT_EQ(devices::id::DeviceType_Interrogate.numeric_id(), 133u);  // runtime method
  EXPECT_EQ(devices::id::TransmissionItemType_SourceAddress.numeric_id(), 225u);
  EXPECT_EQ(devices::numeric_id::Iec60870DeviceType_InterrogationPeriodGroup1, 200u);

  // data_items::
  EXPECT_EQ(data_items::id::DataItemType.numeric_id(), 239u);
  EXPECT_EQ(data_items::id::Statistics_TotalCPUUsage.numeric_id(), 228u);  // runtime metric
  ...
  EXPECT_EQ(security::id::Configurations_DeployConfiguration.numeric_id(), 365u);
