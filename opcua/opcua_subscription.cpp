...
#include "opcua/opcua_monitored_item.h"
...
std::shared_ptr<OpcUaSubscription> OpcUaSubscription::Create(
  ...
  return std::shared_ptr<OpcUaSubscription>(new OpcUaSubscription(session));
}

OpcUaSubscription::OpcUaSubscription(OpcUaSession& session)
    : session_{session} {}
...
void OpcUaSubscription::EnsureCreated() {
  ...
  CoSpawn(session_.any_executor(),
          [this, weak_self]() mutable -> Awaitable<void> {
            ...
            opcua::OpcUaSubscriptionParameters params{
...
void OpcUaSubscription::StartPublishLoop() {
...
void OpcUaSubscription::Subscribe(std::uint32_t local_id,
                                   const scada::ReadValueId& read_value_id,
                                   const scada::MonitoringParameters& params,
  ...
  CoSpawn(
      ...
      [this, local_id, read_value_id, params,
       ...
       weak_self]() mutable -> Awaitable<void> {
        ...
        // queues it).
        opcua::OpcUaMonitoringParameters monitoring{
...
void OpcUaSubscription::Unsubscribe(std::uint32_t local_id) {
  if (!impl_) {
