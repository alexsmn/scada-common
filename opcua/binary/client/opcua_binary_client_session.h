...
// Thin typed facade over OpcUaBinaryClientChannel. Drives the Session
...
// compose them with other coroutines. Errors at any layer (transport,
// secure-channel, decode, service fault, wrong response type) surface as a
// bad Status.
class OpcUaBinaryClientSession {
 public:
  // The session owns the lifetime of transport_, secure_channel_ and
  // channel_ so the caller supplies them pre-constructed but not yet opened.
  struct Context {
  ...
  explicit OpcUaBinaryClientSession(Context context);

  // Runs the connection-establishment dance:
  //   1. transport.Connect()  (HEL/ACK)
  //   2. secure_channel.Open()  (OPN)
  //   3. CreateSession
  //   4. ActivateSession (with the given identity)
  struct Identity {
  ...
  // CloseSession + CloseSecureChannel + transport.Close(), best-effort.
