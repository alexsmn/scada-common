...
class OpcUaBinaryClientSessionTest : public ::testing::Test {
 protected:
...
TEST_F(OpcUaBinaryClientSessionTest, CreateRunsCreateAndActivate) {
...
TEST_F(OpcUaBinaryClientSessionTest, CreatePropagatesCreateSessionBadStatus) {
  auto state = std::make_shared<ScriptedState>();
  PrimeConnectAndOpen(state);
  state->incoming.push_back(AsString(BuildServiceResponseFrame(
      /*request_id=*/2, /*request_handle=*/1,
...
TEST_F(OpcUaBinaryClientSessionTest, ReadReturnsDataValuesOnSuccess) {
  auto state = std::make_shared<ScriptedState>();
  PrimeConnectAndOpen(state);
...
TEST_F(OpcUaBinaryClientSessionTest, WriteReturnsStatusCodes) {
  auto state = std::make_shared<ScriptedState>();
  PrimeConnectAndOpen(state);
  PrimeSessionEstablishment(state);
  state->incoming.push_back(AsString(BuildServiceResponseFrame(
...
TEST_F(OpcUaBinaryClientSessionTest, BrowseReturnsReferences) {
  auto state = std::make_shared<ScriptedState>();
  PrimeConnectAndOpen(state);
  PrimeSessionEstablishment(state);
  state->incoming.push_back(AsString(BuildServiceResponseFrame(
...
TEST_F(OpcUaBinaryClientSessionTest, CallRoundTripsArguments) {
  auto state = std::make_shared<ScriptedState>();
  PrimeConnectAndOpen(state);
  PrimeSessionEstablishment(state);
...
TEST_F(OpcUaBinaryClientSessionTest, CloseRunsCloseSessionBestEffort) {
  auto state = std::make_shared<ScriptedState>();
  PrimeConnectAndOpen(state);
  PrimeSessionEstablishment(state);
  state->incoming.push_back(AsString(BuildServiceResponseFrame(
