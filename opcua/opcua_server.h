...
class OpcUaServer : private OpcUaServerContext {
 ...
 private:
  void Read(OpcUa_ReadRequest& request, const opcua::server::ReadCallback& callback);
  void Browse(OpcUa_BrowseRequest& request, const opcua::server::BrowseCallback& callback);
  opcua::server::CreateMonitoredItemResult CreateMonitoredItem(opcua::ReadValueId& read_value_id);

  const opcua::ByteString server_certificate_;
  const OpcUa_Key server_private_key_{OpcUa_Crypto_KeyType_Invalid, {0, (OpcUa_Byte*)""}};
