...
class OpcUaClientSubscription
    : public std::enable_shared_from_this<OpcUaClientSubscription> {
 public:
  static std::shared_ptr<OpcUaClientSubscription> Create(
      OpcUaClientSession& session);

  OpcUaClientSubscription(const OpcUaClientSubscription&) = delete;
  OpcUaClientSubscription& operator=(const OpcUaClientSubscription&) = delete;
 ...
 private:
  explicit OpcUaClientSubscription(OpcUaClientSession& session);

  void EnsureCreated();
  void StartPublishLoop();

  OpcUaClientSession& session_;
  std::unique_ptr<OpcUaClientProtocolSubscription> impl_;
  ...
  std::unordered_map<std::uint32_t, OpcUaMonitoredItemId>
