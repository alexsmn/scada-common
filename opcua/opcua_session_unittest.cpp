...
class OpcUaSessionTest : public ::testing::Test {
 protected:
...
TEST_F(OpcUaSessionTest, InvalidEndpointRejectsAwaitableWithStatus) {
  auto session = std::make_shared<OpcUaSession>(executor_, transport_factory_);
...
TEST_F(OpcUaSessionTest, InvalidEndpointRejectsLegacyPromiseWithStatus) {
  auto session = std::make_shared<OpcUaSession>(executor_, transport_factory_);
...
TEST_F(OpcUaSessionTest, AwaitableServicesReportDisconnected) {
  auto session = std::make_shared<OpcUaSession>(executor_, transport_factory_);
...
TEST_F(OpcUaSessionTest, LegacyCallbacksUseAwaitableServices) {
  auto session = std::make_shared<OpcUaSession>(executor_, transport_factory_);
