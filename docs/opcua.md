...
| `common/opcua/service_handler.{h,cpp}` | Canonical coroutine-based dispatch from transport-neutral service requests into `CoroutineAttributeService`, `CoroutineViewService`, `HistoryService`, `MethodService`, and `CoroutineNodeManagementService`; legacy callback-service contexts are adapted once at runtime construction boundaries |
