...
HistoryStub::HistoryStub(MessageSender& sender, scada::HistoryService& history_service, std::shared_ptr<Logger> logger) :
    sender_(sender),
...
void HistoryStub::OnRequestReceived(const protocol::Request& request) {
  ...
  scada::ReadValueId read_value_id{
      FromProto(history_read.node_id()),
  ...
  history_service_.HistoryRead(read_value_id, from, to, filter,
      [weak_ptr, this, request_id](Status status, QueryValuesResults values, QueryEventsResults events) {
        if (!weak_ptr.get())
          return;
