...
TEST(Types, Test) {
  ...
  auto* has_component_ref = scada::AsReferenceType(cfg.GetNode(scada::id::HasComponent));
  ASSERT_NE(has_component_ref, nullptr);
  EXPECT_EQ(has_component_ref->GetNodeClass(), scada::NodeClass::ReferenceType);
  EXPECT_TRUE(scada::IsSubtypeOf(*has_component_ref, scada::id::HierarchicalReferences));
}
