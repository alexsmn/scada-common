...
class ConfigurationImpl : public scada::Configuration {
 public:
  explicit ConfigurationImpl(std::shared_ptr<Logger> logger);
...
template <class T>
inline T& ConfigurationImpl::AddStaticNode(std::unique_ptr<T> node) {
  assert(node);
  auto* ptr = node.get();
...
template <class T, class... Args>
inline T& ConfigurationImpl::AddStaticNode(Args&&... args) {
